...
class TestAIGenerator:
    ...
    @patch("backend.ai_generator.anthropic.Anthropic")
    def test_generate_response_single_tool_round(
        ...
        tool_response = MockAnthropicResponse(
            content_blocks=[tool_block], stop_reason="tool_use"
        )
    ...
    @patch("backend.ai_generator.anthropic.Anthropic")
    def test_generate_response_two_tool_rounds(
        self, mock_anthropic, ai_generator, mock_tool_manager
    ):
        ...
        tool_block1 = MockToolUseBlock(
            "get_course_outline", {"course_name": "Python"}, "tool1"
        )
        tool_response1 = MockAnthropicResponse(
            content_blocks=[tool_block1], stop_reason="tool_use"
        )

        # Mock second tool use response
        tool_block2 = MockToolUseBlock(
            "search_course_content", {"query": "lesson 4"}, "tool2"
        )
        tool_response2 = MockAnthropicResponse(
            content_blocks=[tool_block2], stop_reason="tool_use"
        )
    ...
    def test_execute_tool_round(self, ai_generator, mock_tool_manager):
        ...
        tool_block = MockToolUseBlock(
            "search_course_content", {"query": "test"}, "tool123"
        )
        ...
        # Test
        round_result = ai_generator._execute_tool_round(
            mock_response, mock_tool_manager, 1
        )
...
class TestIntegration:
    ...
    def test_end_to_end_sequential_search(self, mock_anthropic):
        ...
        # Mock API responses
        tool_block1 = MockToolUseBlock(
            "get_course_outline", {"course_name": "Course X"}, "tool1"
        )
        response1 = MockAnthropicResponse(
            content_blocks=[tool_block1], stop_reason="tool_use"
        )

        tool_block2 = MockToolUseBlock(
            "search_course_content", {"query": "Advanced Python Decorators"}, "tool2"
        )
        response2 = MockAnthropicResponse(
            content_blocks=[tool_block2], stop_reason="tool_use"
        )

        final_response = MockAnthropicResponse(
            "Course Y discusses the same topic as lesson 4 of Course X"
        )
